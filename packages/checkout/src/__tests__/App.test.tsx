...
test('renders the payment intent route shell, then falls back once the frozen @oxyhq/pay/checkout client rejects', async () => {
  ...
  // Await the settled state within this same test so the `.catch()` state
  // update (a microtask away from the render above) can't leak past this
  // test's completion and land un-`act()`-wrapped on a later test.
