...
function filler(extra = {}) {
  return {
    ...
    ".github/workflows/deploy-aws.yml":
      "        env:\n"
      + "          # MONGODB_URI is deliberately absent: nothing in the image reads it\n"
      + "          TASK_SECRETS: DATABASE_URL\n",
    ...extra,
  };
}
...
const cases = [
  ...
  // ------------------------------------------------------------ manifests ---
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@peable.to/backend", dependencies: { mongoose: "^8.24.0" } },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@peable.to/backend", devDependencies: { "mongodb-memory-server": "^10.0.0" } },
        null,
        2,
      )}\n`,
    }),
  ...
  {
    // The regression this check exists for. `@peable.to/backend` described itself
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@peable.to/backend", description: "Express 5 / Mongoose / Socket.io backend for Mercaria" },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@peable.to/backend", keywords: ["api", "MongoDB"] },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    files: filler({
      "packages/backend/package.json": `${JSON.stringify(
        { name: "@peable.to/backend", description: "Ranked among the fastest, deployed in Mongolia" },
        null,
        2,
      )}\n`,
    }),
  ...
  },
  {
    ...
    // The live list holds exactly one entry, for the deploy workflow's comment.
    // This tree omits that file, so the entry goes stale and the shrink
    // discipline must fire. If the list is ever emptied legitimately, this case
    // stops being able to fail — replace it with the source-level assertion
    // Mercaria's copy uses, rather than weakening it.
    files: {
